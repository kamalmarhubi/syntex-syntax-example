...
fn parse<'a, T: ?Sized + AsRef<Path>>(path: &T,
    ...
    match parse::parse_crate_from_file(path, cfgs, parse_session) {
        // There may be parse errors that the parser recovered from.
...
impl<'v, 'a> Visitor<'v> for CountFnArgs<'a> {
    ...
    // Needed to work on files with macro invocations, eg `format!(...)` above. A better solution
    // would be to expand macros before walking the AST, but that seems messy. This will miss any
    // functions defined via a macro.
